...
public class MFShadowLayout extends FrameLayout {
    ...
    public MFShadowLayout(Context context) {
        this(context, null);
    }

    public MFShadowLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }


    public MFShadowLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttributes(AttributeSet attrs) {
        TypedArray attr = getContext().obtainStyledAttributes(attrs, R.styleable.MFShadowLayout);
        if (attr == null) {
            return;
        }
        ...
        try {
            //默认是显示
            leftShow = attr.getBoolean(R.styleable.MFShadowLayout_hl_leftShow, true);
            rightShow = attr.getBoolean(R.styleable.MFShadowLayout_hl_rightShow, true);
            bottomShow = attr.getBoolean(R.styleable.MFShadowLayout_hl_bottomShow, true);
            topShow = attr.getBoolean(R.styleable.MFShadowLayout_hl_topShow, true);
            mCornerRadius = attr.getDimension(R.styleable.MFShadowLayout_hl_cornerRadius, getResources().getDimension(R.dimen.dp_0));
            //默认扩散区域宽度
            mShadowLimit = attr.getDimension(R.styleable.MFShadowLayout_hl_shadowLimit, getResources().getDimension(R.dimen.dp_5));

            //x轴偏移量
            mDx = attr.getDimension(R.styleable.MFShadowLayout_hl_dx, 0);
            //y轴偏移量
            mDy = attr.getDimension(R.styleable.MFShadowLayout_hl_dy, 0);
            mShadowColor = attr.getColor(R.styleable.MFShadowLayout_hl_shadowColor, getResources().getColor(R.color.default_shadow_color));
            mBackGroundColor = attr.getColor(R.styleable.MFShadowLayout_hl_shadowBackColor, getResources().getColor(R.color.default_shadowback_color));
        } finally {
            attr.recycle();
        }
    }
